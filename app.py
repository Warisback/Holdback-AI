...
def _page(body: str) -> str:
    ...
    return (
        ...
        + nav("/forecast", "Forecast") + nav("/reminders", "Reminders")
...
# --- feature: escalating release-reminder drafts (12/6/3/1 months before release) -------
...
def _reminder_email(name, amount, tranche, org, due_str, n):
    ...
    close = f"\n\nBest regards,\n{org}"
    if n >= 12:
        return (f"Retention release scheduled — {amount} — {name}",
                f"Hi {name},\n\nA note that {amount} of {tr} is scheduled for release on {due_str}. "
                f"Nothing is needed from you yet — we'll be in touch nearer the date.{close}")
    if n >= 6:
        return (f"Retention release in ~6 months — {amount}",
    ...
    if n >= 3:
        return (f"Retention release approaching — {amount} — due {due_str}",
                f"Hi {name},\n\n{amount} of {tr} is due for release on {due_str}. We'll process it "
    ...
    return (f"Retention due for release next month — {amount}",
            f"Hi {name},\n\n{amount} of {tr} is due for release on {due_str}. We're preparing to "
            f"release it — please confirm your bank details so payment isn't delayed.{close}")


@app.route("/reminders")
def reminders():
    """Escalating release-reminder drafts for each held retention bill. Drafts open in the
    user's mail client via mailto (no email server) — timed auto-send is a post-demo add-on."""
    ...
    for i in held:
        ...
        chips = "".join(
            ...
            f"{n}mo &middot; {sd.strftime('%d %b %y')}</span>" for n, sd in sched)
        ...
        sub = f"releases {due_str}" + (f" &middot; tranche {i['tranche']}" if i["tranche"] else "")
        rows += (
            f"<article class='card'><div class='card-top'><div style='min-width:0'>"
            f"<div class='card-name'>{i['name']}</div><div class='card-sub'>{sub}</div></div>"
            f"<div class='card-amt'>{_money_fmt(i['held'])}</div></div>"
            f"<div class='rem-sched'>{chips}</div>"
            f"<div class='card-meta'>{state}"
            f"<a class='btn btn-primary' style='margin-left:auto;padding:6px 14px' "
            f"href=\"{_mailto(emails.get(i.get('contact_id'), ''), subj, body)}\">"
            "Draft reminder email</a></div></article>"
        )
    rows = rows or "<div class='empty'>No held retention to remind on yet.</div>"
    return (
        "<div class='page-head'><div><h1>Release reminders</h1>"
        "<p class='sub'>An escalating cadence &mdash; 12, 6, 3 and 1 month before each release "
        "&mdash; so retention never slips. HoldBack drafts each email; click to open it in your "
        "mail app.</p></div></div>"
        f"{rows}"
        "<p class='hint' style='margin-top:14px'>Drafts open in your email client (no mail server "
        "needed). Automatic timed sending would need an email provider + a scheduler &mdash; a "
        "post-demo add-on.</p>"
    )
